...
function targeting.updateRushTarget(player, standDef, standEntity, standData, opts)
    ...
    if not standData.tgt and opts.allowGrid ~= false and Settings.TargetGridEntities then
...
function targeting.updateAttackTarget(player, standDef, standEntity, standData)
    ...
    if standData.punches < standData.maxpunches
        and not (standChecks:IsValidEnemy(standData.tgt, player, standEntity) or standChecks:IsTargetable(standData.tgt, player, standEntity))
...
function targeting.forEachValidEnemy(player, standEntity, fn)
    for _, en in ipairs(RoomEntities.Get()) do
        if standChecks:IsValidEnemy(en, player, standEntity) then
            if fn(en) == false then
                return
